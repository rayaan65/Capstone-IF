...
def verify_token():
    """Verify Firebase token and create a server session"""
    data = request.json
    ...
    if result['success']:
        ...
        session['user_id'] = result['email'] or result['user_id']
        session['name'] = result.get('name', '')
        session['picture'] = result.get('picture', '')
        session['custom_token'] = result.get('session_token', '')
        session.permanent = True
        
        # Create user record if not exists
        email = result['email'] or result['user_id']
        ...
        return jsonify({'success': False, 'error': result.get('error', 'Authentication failed')}), 400
    ...
    if request.method == 'POST':
        ...
        if email in users and check_password_hash(users[email]['password'], password):
            ...
            user_data = {
                'uid': email,
                'email': email,
                'displayName': users[email].get('name', '')
            }
            custom_token = create_custom_token(user_data)
            
            # Set session data
            session.clear()
            session['user_id'] = email
            session['name'] = users[email].get('name', '')
...
def analyze():
    ...
    # Refresh the session to prevent timeout during long operations
    session.modified = True
    ...
    if analysis_type == 'summary':
        ...
        # Optimize correlation calculation by:
        # 1. Using a sample of the data if rows > 10000
        if len(numeric_df) > 10000:
            # Sample 10,000 rows for faster calculation
            numeric_df = numeric_df.sample(n=10000, random_state=42)
            if columns_message:
                columns_message += " Data sampled to 10,000 rows for performance."
            else:
                columns_message = "Data sampled to 10,000 rows for performance."
        
        # 2. Drop columns with too many missing values
        missing_threshold = 0.5  # 50% missing values
        cols_to_keep = numeric_df.columns[numeric_df.isnull().mean() < missing_threshold]
        if len(cols_to_keep) < len(numeric_df.columns):
            numeric_df = numeric_df[cols_to_keep]
            if columns_message:
                columns_message += " Columns with >50% missing values were excluded."
            else:
                columns_message = "Columns with >50% missing values were excluded."
        
        # 3. Use pairwise deletion for missing values instead of dropping entire rows
        corr_matrix = numeric_df.corr(method='pearson', min_periods=1).to_dict()
        ...
        try:
            ...
            mask = np.triu(np.ones_like(numeric_df.corr(min_periods=1), dtype=bool))
            
            # Generate heatmap with seaborn - with optimized settings
            sns.heatmap(
                numeric_df.corr(min_periods=1),
                mask=mask,
                annot=True,
                fmt=".2f",
                cmap="coolwarm",
            ...
            plt.figure(figsize=(10, 8))
            plt.matshow(numeric_df.corr(min_periods=1), fignum=1)
            plt.colorbar()
        ...
        try:
            ...
            try:
                ...
                if chart_type == 'pie':
                    ...
                    # Create a figure with more room for labels
                    ...
                    plt.xlabel(f'{category_col}', fontsize=12)
                    ...
                    # Add 20% padding at the top to accommodate labels
                    ...
                    for i, val in enumerate(values):
                        ...
                        # Position labels closer to the data points
                        y_pos = val + (y_max - y_min) * 0.05
                ...
                # Ensure proper padding - use larger padding for line charts
                if chart_type == 'line':
